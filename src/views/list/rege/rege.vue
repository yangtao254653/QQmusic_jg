<template>
  <div>
    <div class="top">
      <h3>热歌榜</h3>
      <span class="time">3.25-03.31</span>
      <span>榜单规则</span>
    </div>
    <div class="btns">
      <button>播放全部</button>
        ...
        <ul class="bottom-list1">
          <li v-for="(item, index) in arrs" :key="index">
            <div class="basis-left">
              ...
              <span class="shangsheng">{{ item.shangsheng }}</span>
              <img :src="item.imgUrl" alt="" class="image" />
              <span class="geming">{{ item.geming }}</span>
            </div>
...
export default {
  ...
  methods: {
    async getdata() {
      let { data } = await instance.get("/rege");
      console.log(1);
      console.log(data);
      this.arrs = data;
